...
add_analytics = True
# for v in [0., 10., 50., 100., 200., 500.]:
#     DATA_PATH = f"data/msd_a{alpha}_T{T}_h{h}_v{v}_mu{mu}_r{realizations}.npz"
#     eq = afbm(alpha, T, h, v, mu, B_T, B_R, KBT)
#     plot.plot_msd(ax=ax,eq=eq, component="r", add_analytics=add_analytics, label=fr"$v={eq.v}$", data_path=DATA_PATH)
#     ax.set_xscale("log")    
#     ax.set_yscale("log")
...
for v in [0., 10., 50., 100., 200., 500.]:
    ...
    DATA_PATH = f"data/msd_a{alpha}_T{T}_h{h}_v{v}_mu{mu}_r{realizations}.npz"
    eq = afbm(alpha, T, h, v, mu, B_T, B_R, KBT)
    plot.plot_msd(ax=ax,eq=eq, component="phi", add_analytics=add_analytics, label=fr"$v={eq.v}$", data_path=DATA_PATH)

    ax.set_xscale("log")    
    ax.set_yscale("log")

plot.add_trend(ax, tmin=1, tmax=200, exponent=0.9, yref=1, tref=1, color='blue', linestyle='--')
